...
def describe_int_diffs(dfx_int_obs, dfx_int_exp, mask_int, n_rows=5):
    ...
    #return f'first {n_rows} rows of int diffs:\nobserved:\n{repr(df_prev_obs)}\nexpected:\n{repr(df_prev_exp)}'
...
def df_compare(df_obs, df_exp, n_show=5, *args, **kwargs):
    ...
    Compares the following features of the two DataFrames
      ...
      6. floats ('float'): Select float columns and compare with numpy.is_close
      7. Datetimes: ('datetime'): Select date-like columns and compare by equality.
      8. Objects / strings ('object'): Select object columns and compare by equality.
      9. NaNs ('nan'): Compare location of NaNs
    ...
    # 5. Integers
    dfx_int_obs = dfx_obs.select_dtypes(include=[np.int])
    dfx_int_exp = dfx_exp.select_dtypes(include=[np.int])
